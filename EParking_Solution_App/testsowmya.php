...
//  $sqlsss="SELECT COUNT(*) FROM `table_bookings` WHERE parking_id='$pid' AND booking_time='$tim' AND booking_date='$dat' ";
//   $resultssss = $con->query($sqlsss);
...
if ($resultdata->num_rows > 0) {
    ...
    $rowdata = $resultdata->fetch_assoc();
  // output data of each row
  $parkings= $rowdata[toal_parkings];
  //number of totalparkings
  echo $parkings;
  
  if($parkings<=$bookings)
  {
